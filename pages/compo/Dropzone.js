...
const Dropzone = () => {
    ...
    const uploadPost = async () => {
      ...
      await Promise.all(
        selectedImage.map(image => {
          const imageRef = ref(storage, `gallery/${docRef.id}/${image.path}`);
    ...
    const onDrop = useCallback(acceptedFiles => {
      ...
      return (
        <div className=' border-dotted border-2 border-sky-500 p-2  shadow-xl ' >
        <div {...getRootProps({style})}>
          <input className='  m-2 bg-white bg-opacity-25 placeholder-black font-bold p-4  w-full  h-[200px] border-dotted border-2 border-sky-500 ' {...getInputProps()} />
              <p  >gallery Drop Zone </p>
              <PlusCircleIcon className="   h-8 cursor-pointer text-purple-500 "  /> 
              {selected_images}
        </div>
        
        <button className="block ml-auto mr-auto   justify-center w-full  text-center xl:w-[200px] px-8 py-2   my-2 hover:shadow-xl shadow-md   rounded-md active:scale-90 transition duration-150 text-white bg-purple-500"  onClick={uploadPost}>Upload To Gallery</button>
