...
const FourthbannerDrop = () => {
    ...
    const uploadPost = async () => {
      const docRef = await addDoc(collection(db,"fourthbanner"),{
        
        timestamp:serverTimestamp()
      })
      await Promise.all(
        selectedImage.map(image => {
          const imageRef = ref(storage, `fourthbanner/${docRef.id}/${image.path}`);

          uploadBytes(imageRef,image,"data_url").then(async()=>{
            const downloadURL = await getDownloadURL(imageRef)
            await updateDoc(doc(db,"fourthbanner",docRef.id),{
...
export default FourthbannerDrop