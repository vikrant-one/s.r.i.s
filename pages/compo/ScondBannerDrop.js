...
const ScondBannerDrop = () => {
    ...
    const uploadPost = async () => {
      const docRef = await addDoc(collection(db,"secondbanner"),{
      
        timestamp:serverTimestamp()
      })
      await Promise.all(
        selectedImage.map(image => {
          const imageRef = ref(storage, `secondbanner/${docRef.id}/${image.path}`);

          uploadBytes(imageRef,image,"data_url").then(async()=>{
            const downloadURL = await getDownloadURL(imageRef)
            await updateDoc(doc(db,"secondbanner",docRef.id),{
...
export default ScondBannerDrop