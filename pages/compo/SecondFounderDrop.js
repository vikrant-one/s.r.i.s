...
const SecondfounderDrop = () => {
    ...
    const uploadPost = async () => {
      const docRef = await addDoc(collection(db,"secondfounder"),{
      ...
      await Promise.all(
        selectedImage.map(image => {
          const imageRef = ref(storage, `secondfounder/${docRef.id}/${image.path}`);

          uploadBytes(imageRef,image,"data_url").then(async()=>{
            const downloadURL = await getDownloadURL(imageRef)
            await updateDoc(doc(db,"secondfounder",docRef.id),{
...
export default SecondfounderDrop