...
const ThirdfounderDrop = () => {
    ...
    const uploadPost = async () => {
      const docRef = await addDoc(collection(db,"thirdfounder"),{
      ...
      await Promise.all(
        selectedImage.map(image => {
          const imageRef = ref(storage, `thirdfounder/${docRef.id}/${image.path}`);

          uploadBytes(imageRef,image,"data_url").then(async()=>{
            const downloadURL = await getDownloadURL(imageRef)
            await updateDoc(doc(db,"thirdfounder",docRef.id),{
...
export default ThirdfounderDrop